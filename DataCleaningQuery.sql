-- Social buzz data cleaning script
-- I used MSSQL for this task.
-- This task is strictly about cleaning each of the three datasets provided.

-- Creating Schema
CREATE  SCHEMA socialbuzz;

/*Although I wrote a query to create the schema for this task, going forward,
I will approach the task like I am a junior Data Analyst who has been restricted from using most DDL and DML languages when working with this database, 
...
Oftentimes, intern and junior analysts are restricted from such administrator privileges and only allowed to use mainly query language and commands,
so, it helps to get used to how to get the task done in spite of this restriction
...
However, a simple SELECT statement with a WHERE clause where necessary, works in this case. 
...
- remove columns that are outrightly redundant- like column1 in all the datasets.
The column header was auto-generated from a user-defined index in the csv file during import into the database.
and was not initially so named in the orginal dataset
-Collect out a subset from each dataset, of columns relevant to the task

Datatypes were correctly infered at import so, all the columns are in the right datatype

In order to avoid creating more than one resulting output and having to query the result again to achieve the final cleaned dataset, 
I will work towards achieving all the aforelisted cleaning processes in one query for each of the datasets
*/

/* Cleaning the Content table
The main dirty data issues to address here are:
- removing rows with null values and doing away with column1. 
I am going to remove the null values without using the 'DELETE' statement- No DDLs and DMLs, remember. I will only use the CREATE statement to create the views in this work
...
I will also do some data transformation, like renaming some columns to make them better descriptive, where needed.
...
-- There is value inconsistency in the column. Some of the categories have multiple variants due to some values having double quotes.
...
-- I used 'Script table as' to see the table description, so as to identify columns not constrained as 'Not Null' as these are the columns that could introduce null values.
...
-- Next, let's work on the 'Reactions' data.
...
--LEFT(CONVERT(DATETIME2(0), Datetime), RIGHT(Datetime, 3)) 
FROM socialbuzz.Reactions
WHERE Type IS NOT NULL-- This is the only column needed that has null values so, I used it in my WHERE clause.

-- Lastly, let's work on the 'ReactionTypes' data.
-- We'll only take out Column1 from this dataset. We'll keep every other column and rename 'Type' as 'Reaction_Type'
...
--Exporting a table or view in the MSSQL database used was appending new rows to existing rows in the csv file 
--and did not overide existing rows. I hope to investigate this and find a solution soon. 
...
-- We will query the views earlier created to achieve this.
...
FROM socialbuzz.CleanedReactions AS r
JOIN socialbuzz.CleanedContent AS c -- Inner Join
ON r.Content_ID = c.Content_ID) -- the subquery
...
but I'll like to do this in a Spreadsheet, just to demonstrate my ability to work with different technologies (Excel or Google Sheet can be used. I'll be using Google Sheets- *wihispers* - my Excel access has expired)
So, I will end here by exporting my view into a CSV file in preparation to continue in a Spreadsheet.
...
-- I am sharing the formulas here because you will not find any formula in my final spreadsheet. 
-- It is good practice to always copy and paste the results as values only after applying a formula or function, so it does not interfere in our analysis.

-- Also note that the aggregating and sorting process can also be achieved with pivot tables.
